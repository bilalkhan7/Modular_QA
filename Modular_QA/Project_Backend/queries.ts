...
// request('http://localhost:5000/sql_generate', function (error, response, body) {
...
const getquery = (req, res, next) => {
  ...
  const received_json = res.send(JSON.stringify({jsondata}));
  console.log('json received from frontend', received_json);

  request.get("http://localhost:5000/sql_generate", (error, response, body) => {
    if(error) {
        return console.dir(error);
    }
...
// testing json return format for sample query
...
// test json formatting for graph  

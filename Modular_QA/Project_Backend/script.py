...
def main():
                ...
                if 'rules' in list(sample.keys()):
                    ...
                    for each in sample['rules'][:-1]:
                        ...
                        if "value" in list(each.keys()):
                            ...
                            main_frame = main_frame.append(
                                temp, ignore_index=True, verify_integrity=True)
    ...
    join_frame = pd.DataFrame(columns=list(
        table_frame['table']), index=list(table_frame['table']))

    for x in list(table_frame['table']):
        for y in list(table_frame['table']):
            join_frame.loc[x, y] = check_lists(table_frame[table_frame.table == x]['columns'].tolist()[
                                               0], table_frame[table_frame.table == y]['columns'].tolist()[0])
    ...
    if len(data.keys()) <= 1:
        ...
        query = query + col_list + \
            keywords[1] + str(list(main_frame["table"])[0]) + keywords[2]
    ...
    # query with two or more than two tables
    else:
        ...
        for x in range(len(tables_list)):
            for y in range(x+1, len(tables_list)):
                ...
                if len(join_frame.loc[tables_list[x], tables_list[y]]) > 0:
                    temp += [table1, table2]
                    l1 = str(main_frame[main_frame.table == table1]
                             ['reference'].drop_duplicates().tolist()[0])
                    l2 = str(main_frame[main_frame.table == table2]
                             ['reference'].drop_duplicates().tolist()[0])

                    query_part = query_part + keywords[3] + tables_list[x+1] + " " + str(main_frame[main_frame.table == tables_list[x+1]]['reference'].drop_duplicates(
                    ).tolist()[0]) + keywords[4] + l1+'.' + str(join_frame.loc[table1, table2][0]) + "=" + l2+'.' + str(join_frame.loc[table1, table2][0])

        # editing main_frame according to join possibility
        ...
        for row_number in list(main_frame.index):
            col_list = col_list + \
                main_frame.iloc[row_number]['reference'] + \
                '.' + main_frame.iloc[row_number]['field'] + ','
        ...
        # adding other kwywords and table name
        query = query + ' ' + col_list + \
            keywords[1] + str(list(main_frame["table"])[0]) + \
            " " + str(list(main_frame["reference"])[0])
        ...
        for row_number in list(main_frame.index):
            ...
            if row_number == 0:
                query = query + \
                    main_frame['reference'].iloc[row_number] + \
                    '.'+main_frame['field'].iloc[row_number]
                for each in list(main_frame[['operator', 'value']].iloc[row_number]):
                    query = query + " " + each
            else:
                query = query + ' '+main_frame['condition'].iloc[row_number]+" " + \
                    main_frame['reference'].iloc[row_number] + \
                    '.'+main_frame['field'].iloc[row_number]
                for each in list(main_frame[['operator', 'value']].iloc[row_number]):
                    #                query = query + main_frame['reference'].iloc[row_number] +'.'+main_frame['field'].iloc[row_number]
